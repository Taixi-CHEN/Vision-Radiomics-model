...
class XDecoder_Trainer(DefaultTrainer):
    ...
    def create_optimizer_and_scheduler(self):
        ...
        for _module_name in self.model_names:
            ...
            actual_model = self.raw_models[_module_name].model
            if hasattr(actual_model, 'get_jamba_parameters'):
                jamba_params = actual_model.get_jamba_parameters()
                if jamba_params:  # 检查是否有参数
                    for jamba_param in jamba_params:
                        if not jamba_param.requires_grad:
                            continue
                        if jamba_param in memo:
                            continue
                        memo.add(jamba_param)
                        
                        hyperparams = copy.copy(defaults)
                        # Jamba参数使用默认学习率
                        params.append({"params": [jamba_param], **hyperparams})
                        if is_main_process():
                            print("yes, finetune Jamba")
                            logger.info(f"Added Jamba parameter to optimizer: {jamba_param.shape}")
                        else:
                            print("no finetune jamba (not main process)")
                else:
                    if is_main_process():
                        print("Jamba model exists but has no parameters")
            else:
                if is_main_process():
                    print("no fine jamba at initial - model has no get_jamba_parameters method")
