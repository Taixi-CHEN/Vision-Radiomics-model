...
class DefaultTrainer(UtilsTrainer, DistributedTrainer):
    ...
    def _eval_on_set(self, save_folder, epoch=None):
        logger.info(f"Evaluation start ...")
        if self.opt['FP16']:
            # from torch.cuda.amp import autocast
            from torch import amp
            device_type = 'cuda' if self.opt.get('CUDA', False) else 'cpu'
            with amp.autocast(device_type):
                results = self.pipeline.evaluate_model(self, save_folder, epoch)
        else:        
            results = self.pipeline.evaluate_model(self, save_folder, epoch)
        if self.opt['rank'] == 0:
            logger.info(results)
    ...
    def init_train(self):
        ...
        # compute optimizer updates per epoch without relying on len(dataloader)
        updates_per_epoch = self.opt.get("LR_SCHEDULER_PARAMS", {}).get("steps_update_per_epoch")
        if updates_per_epoch is None:
            train_names = self.opt['DATASETS']['TRAIN']
            if isinstance(train_names, str):
                train_names = [train_names]
            num_imgs = sum(len(DatasetCatalog.get(n)) for n in train_names)

            if "TRAIN" in self.opt and "BATCH_SIZE_TOTAL" in self.opt["TRAIN"]:
                batch_total = int(self.opt["TRAIN"]["BATCH_SIZE_TOTAL"])
            else:
                per_gpu = int(self.opt.get("TRAIN", {}).get("BATCH_SIZE_PER_GPU", 1))
                world   = int(self.opt.get("world_size", 1))
                batch_total = max(1, per_gpu * world)

            grad_acc = int(self.opt.get("GRADIENT_ACCUMULATE_STEP", 1))
            steps_epoch  = math.ceil(num_imgs / batch_total)
            updates_per_epoch = math.ceil(steps_epoch / max(1, grad_acc))

        self.opt.setdefault("LR_SCHEDULER_PARAMS", {})["updates_per_epoch"] = updates_per_epoch
    ...
    def train(self):
        ...
        if self.opt.get('EVAL_AT_START', False):
            # results = self._eval_on_set(self.save_folder)
            results = self._eval_on_set(self.save_folder, epoch=0)
        ...
        for epoch in range(self.train_params['start_epoch_idx'], num_epochs):
            ...
            warmup_epochs = 0
            for module_name in self.model_names:
                ddp_wrapped = self.models[module_name]
                # Unwrap DDP -> BaseModel -> underlying nn.Module
                target = getattr(ddp_wrapped, 'module', ddp_wrapped)
                target = getattr(target, 'model', target)
                if hasattr(target, 'enable_radiomics_fusion'):
                    target.enable_radiomics_fusion = (epoch >= warmup_epochs)
                    if epoch == warmup_epochs and self.opt['rank'] == 0:
                        print('begin using radiomics fusion...')
                else:
                    if self.opt['rank'] == 0:
                        print('no radiomics fusion flag on model')
            ...
            for batch_idx, batch in enumerate(self.train_dataloaders):
                ...
                if batch_idx + 1 == self.train_params['updates_per_epoch']:
                    ...
                    # results = self._eval_on_set(self.save_folder)
                    results = self._eval_on_set(self.save_folder, epoch=epoch)
                    # if self.opt['rank'] == 0 and self.opt['WANDB']:
                    #     wandb.log(results)
                    break
